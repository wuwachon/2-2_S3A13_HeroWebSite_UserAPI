...
const heroDatas = JSON.parse(localStorage.getItem('favoriteHero')) || [];
const heroPerPage = 6;
let nowPage = 1
const superpower = [
  {
...
function renderGridDataPanel(herodata) {
  const dataFromLocalStorage = JSON.parse(localStorage.getItem('heroNames')) || []
  let rawHTML = ''
  for (let info of herodata) {
    const favorite = (dataFromLocalStorage.some(heroName => heroName === info.name)) ? 'favorite' : 'favorite_border'
    rawHTML += `
    <div class="col-sm-2">
